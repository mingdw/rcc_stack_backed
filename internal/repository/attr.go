...
func (r *attrRepository) DeleteByAttrGroupID(ctx context.Context, id int64) error {
	return r.DB(ctx).Where("attr_group_id = ? and is_deleted = 0  and  attr_type = 0", id).Delete(&model.Attr{}).Error
}

func (r *attrRepository) GetAttrByCode(ctx context.Context, code string) (*model.Attr, error) {
	var attr model.Attr
	if err := r.DB(ctx).Where("attr_code = ? and is_deleted = 0", code).First(&attr).Error; err != nil {
...
func (r *attrRepository) Create(ctx context.Context, attr *model.Attr) error {
	return r.DB(ctx).Create(attr).Error
}

func (r *attrRepository) Update(ctx context.Context, attr *model.Attr) error {
	return r.DB(ctx).Model(&model.Attr{}).Where("id = ? and is_deleted = 0", attr.ID).Select("status", "sort", "attr_type", "updated_at", "updator").Updates(attr).Error
}

func (r *attrRepository) Delete(ctx context.Context, id int64) error {
	return r.DB(ctx).Where("id = ? and is_deleted = 0", id).Delete(&model.Attr{}).Error
}
